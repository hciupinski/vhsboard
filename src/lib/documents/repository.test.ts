...
beforeEach(() => {
  ...
  mockedSupabase.storage.from.mockReturnValue({
    ...
    createSignedUrls: vi.fn().mockResolvedValue({
      data: [{ path, signedUrl: "https://signed.example/regulamin.pdf" }],
      error: null,
    }),
  });
});
...
describe("contact document repository", () => {
  it("lists metadata with a readable signed URL", async () => {
    ...
    await expect(listPublicContactDocuments()).resolves.toEqual([
      {
        id: documentId,
        title: "Regulamin",
        path,
  ...
  it("uploads the PDF before inserting validated metadata", async () => {
    ...
    await uploadContactDocument(
      "  Regulamin  ",
      new File(["pdf"], "regulamin.pdf", { type: "application/pdf" }),
      0,
    );
  ...
  it("cleans up the object after metadata insert failure", async () => {
    ...
    ).rejects.toThrow("nie udało się zapisać");
  ...
  it("reports a retryable cleanup failure after deleting metadata", async () => {
    ...
    await expect(deleteContactDocument(documentId)).rejects.toBeInstanceOf(
      DocumentCleanupPendingError,
    );
  });

  it("does not retry an arbitrary Storage path", async () => {
    await expect(retryContactDocumentObjectCleanup("private/file.pdf")).rejects.toThrow(
