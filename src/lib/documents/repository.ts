...
const parseRow = (value: unknown): ContactDocumentRow => {
  try { return contactDocumentRowSchema.parse(value); }
  catch (cause) { throw new DocumentRepositoryError("Nie udało się odczytać dokumentu.", { cause }); }
};

const toDocument = (value: unknown, signedUrl: string | null = null): ContactDocument => {
  const row = parseRow(value);
  return { id: row.id, title: row.title, path: row.storage_path, position: row.position, signedUrl };
};
...
const signedUrlsFor = async (paths: string[]): Promise<Map<string, string>> => {
  ...
  if (error || !Array.isArray(data)) throw new DocumentRepositoryError("Nie udało się przygotować odnośników do dokumentów.", { cause: error });
  return new Map(data.flatMap((item) => {
    if (typeof item.path !== "string" || typeof item.signedUrl !== "string") return [];
    try { return new URL(item.signedUrl).protocol === "https:" ? [[item.path, item.signedUrl] as const] : []; }
    catch { return []; }
  }));
};

const listContactDocuments = async (): Promise<ContactDocument[]> => {
  const { data, error } = await supabase.from("contact_documents").select(COLUMNS).order("position", { ascending: true });
  if (error || !Array.isArray(data)) throw new DocumentRepositoryError("Nie udało się pobrać dokumentów.", { cause: error });
...
export const uploadContactDocument = async (title: string, file: File, position: number): Promise<ContactDocument> => {
  ...
  if (!Number.isInteger(position) || position < 0) throw new DocumentRepositoryError("Nieprawidłowa pozycja dokumentu.");
  const path = createDocumentPath(crypto.randomUUID());
  const storage = supabase.storage.from(BUCKET);
  const { error: uploadError } = await storage.upload(path, new Blob([await file.arrayBuffer()], { type: file.type }), { contentType: file.type, upsert: false });
  if (uploadError) throw new DocumentRepositoryError("Nie udało się wgrać dokumentu.", { cause: uploadError });
  const { data, error } = await supabase.from("contact_documents").insert({ title: validTitle.value, storage_path: path, position }).select(COLUMNS).single();
  if (error) {
    try { await storage.remove([path]); } catch { /* compensation is best effort */ }
    throw new DocumentRepositoryError("Dokument został wgrany, ale nie udało się zapisać go na liście.", { cause: error });
  }
  return toDocument(data);
};

const loadDocument = async (id: string): Promise<ContactDocumentRow> => {
  const { data, error } = await supabase.from("contact_documents").select(COLUMNS).eq("id", id).maybeSingle();
  if (error || data === null) throw new DocumentRepositoryError("Dokument nie istnieje.", { cause: error });
...
export const deleteContactDocument = async (id: string): Promise<void> => {
  ...
  if (error) throw new DocumentRepositoryError("Nie udało się usunąć dokumentu z listy.", { cause: error });
